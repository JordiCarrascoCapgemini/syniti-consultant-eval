...
- **DEC-10 - Deployment mechanism: Portainer.** DECIDED (2026-07-28). Application stacks are
  deployed and operated through **Portainer CE using git-backed stacks**, so deploying an app
  does not require shell access on the host, and secrets are entered as Portainer stack
  environment variables rather than a `.env` file on disk. Portainer itself is the one stack
  deployed from the CLI (`deploy/portainer/docker-compose.yml`) because it cannot deploy itself.
  Its UI sits **behind nginx on its own FQDN** like any other vhost, published on loopback only.
  **Portainer mounts the Docker socket and is therefore root-equivalent on the host**; the
  mitigations (loopback binding, IP allow-list on its vhost, pinned image, no app CSP applied)
  are in `deploy/DEPLOY-POCDAPP314.md` section 9. A CLI fallback is documented for the case
  where the host cannot reach the git remote.
- **DEC-9 - Hosting and ingress on POCDAPP314.** DECIDED (2026-07-28). The stack runs on the
  single POCLAB Linux host **POCDAPP314 (10.21.12.62)** with a **host-level nginx** as shared
  reverse proxy. Two sub-decisions: (a) **shared IP, one FQDN per app**, routed on Host header
  and TLS SNI - per-app IP addresses were considered and rejected as adding coordination
  without isolation; (b) **internal CA certificates**, one per hostname. nginx is deliberately
  NOT part of any application's compose stack so that restarting one app cannot drop ingress
  for the others. Spec and runbook: `deploy/DEPLOY-POCDAPP314.md`; architecture:
  `deploy/architecture.html`.
- **DEC-7 - Repository home for the tool.** DECIDED (2026-07-28). This repo is **canonical
  again** for the tool and its new server. The v6.1 reconciliation homed the project in the
  monorepo and marked this repo archive-only; a deployable Docker service with a Postgres
  volume is a poor fit inside a notes monorepo, so the tool moves back here. The
  Reconciliation note below is updated accordingly.
- **DEC-8 - Dockerized deployment shape.** DECIDED (2026-07-28). Fourteen design decisions
  (additive server, leads-only shared pool, JSONB document storage, files stay master for
  reference data, FastAPI, server-side session cookie, runtime feature detection,
  append-only writes, compose with app plus Postgres, seed command plus integration test,
  minimal UI, both apps built by `build.py`, plain idempotent SQL) are recorded in full in
  **`CLAUDE.md`**, which is the reference. Do not relitigate one without saying which.
...
- **LIM-5 - Multiple copies of COMPS / LEARNING.** CLOSED (2026-07-28). Both apps are now
  generated by `build/build.py` from `data/*.json` (`build/template.html` and
  `build/template-team.html`), and the short-key conversion lives once in `refdata.py`, shared
  by the builder and the server's `/api/reference`. The team app is no longer hand-edited.
  Historical description follows. The eval tool embeds both consts (rebuilt from
...
- **LIM-7 - No browser storage.** Still true, and still deliberate, for the portable file.
  The dockerized deployment adds optional server-side persistence in Postgres instead of
  browser storage, and the apps feature-detect it (offline behaviour is unchanged; opened
  from `file://` they make no network call at all). Original note: the tool intentionally
  keeps all state in memory and persists via
...
- **ENH-5 - Single source for the catalog.** DONE (2026-07-28) for both the build and the
  served path: `refdata.py` is the one loader, used by `build/build.py` and by
  `GET /api/reference`. `data/learning-catalog.json` is the only place to edit.
...
- **Un-archived (2026-07-28).** DEC-7 makes this repo canonical again for the tool and the
  new server (`server/`, `db/`, `Dockerfile`, `docker-compose.yml`). The 2026-07-17 note
  below is superseded on the question of which home is canonical; it is kept for history.
