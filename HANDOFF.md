...
>    competencies were restructured into an **AI Enablement** area (3 items, provisional expectations);
...
- **AI Guided-review section removed.** The tool is now **four sections** (01 Details & summary,
  02 Scope, 03 Skills evaluation, 04 Learning plan). All prompt/paste-back JS and the `reviewMode`
  state are gone. `prompts/Guided review prompt.md` is deprecated.
- **AI competencies restructured** into a new **AI Enablement** area (was "Squad-Specialized and AI
  ...
  Expected values for the three are **provisional - confirm with the CoE**.
...
### 4.1 Design tokens (CSS variables, in `:root`)
```
--navy #1E2761  --blue #2563CC  --violet #8B5CF6  --teal #0E9F6E  --amber #E08A0B  --red #DC5A54
--ink #1E2440   --muted #6B7492 --slate #64748B
--hair rgba(255,255,255,.65)  --glass rgba(255,255,255,.55)  --shadow ...
--grad linear-gradient(120deg,#2563CC,#8B5CF6)   --li #0A66C2 (LinkedIn)  --dg #E08A0B (Degreed)
--maxw 1360px
```
Fonts: Space Grotesk (display/headings), Inter (body), JetBrains Mono (labels, refs, ratings).
Aesthetic: aurora blob background + glass cards (`backdrop-filter: blur + saturate`).
...
- `renderFramework()` - fills the Career Framework reference panel for the selected level (section 01).
...
**Four** cards, ids used by the nav + IntersectionObserver:
...
`#sec-learning` (04 Learning plan). (`#sec-ai` was removed.)
