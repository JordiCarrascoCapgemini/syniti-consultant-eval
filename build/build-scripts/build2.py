...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .crow{display:grid;grid-template-columns:1fr auto;gap:16px;padding:15px 18px;border-top:1px solid rgba(100,116,139,.12);
    align-items:start}
  .cinfo .ct{font-family:'Space Grotesk',sans-serif;font-weight:600;font-size:14px;color:var(--ink)}
  .cinfo .cd{color:var(--muted);font-size:12.5px;line-height:1.45;margin-top:4px;max-width:62ch}
  .cinfo .cref{font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--slate);
    border:1px solid var(--hair);border-radius:5px;padding:1px 6px;margin-right:8px;background:rgba(255,255,255,.5)}
  .crate{display:flex;flex-direction:column;align-items:flex-end;gap:9px;min-width:290px}
  .expbadge{font-family:'JetBrains Mono',monospace;font-size:10.5px;color:var(--slate);white-space:nowrap}
  ...
  @media(max-width:720px){
    ...
    .crate{align-items:flex-start;min-width:0}
    .bar .lbl{width:120px}
  }
  @media print{
    body{background:#fff}
...
<div class="shell">
  ...
  <div class="card scopewrap">
    <div class="panel-h"><span class="n">02</span> Scope the assessment</div>
  ...
  <div class="card">
    <div class="panel-h"><span class="n">03</span> Skills matrix</div>
  ...
  <div class="card">
    <div class="panel-h"><span class="n">04</span> Project feedback &amp; classification</div>
  ...
  <div class="card">
    <div class="panel-h"><span class="n">05</span> Recommended learning <span class="samplebadge">Sample</span></div>
    <div class="panel-sub">Suggested Degreed and LinkedIn Learning to close the gaps found above. This is placeholder content for now - the catalog is built to be deconstructed and curated later.</div>
    <div class="toolbar">
    ...
    <div class="disc"><b>Curation note:</b> course titles, durations and providers here are sample placeholders mapped by competency area. The learning catalog is a single structured object in this file (keyed by area, with room for competency-level overrides) so it can be lifted out and replaced with curated Degreed pathways and LinkedIn Learning links without touching the rest of the tool.</div>
  </div>

  <div class="card">
    <div class="panel-h"><span class="n">06</span> Summary</div>
    <div class="panel-sub">Auto-calculated from the ratings above. Updates live.</div>
    <div class="bandbadge meets" id="bandBadge"><span class="dotm"></span><span id="bandLabel">Meets expectations</span></div>
...
const PROV_URL = {
  ...
  degreed:  q => "https://degreed.com/search?query="+encodeURIComponent(q)
};
const PROV_NAME = {linkedin:"LinkedIn Learning", degreed:"Degreed"};
...
let state = {level:"C3", ratings:{}, na:{}, scope:{}, cmp:null, prov:"both", onlyGaps:true, plan:{}};
...
function renderMatrix(){
  ...
  if(!activeAreas.length){ m.appendChild(el("div","panel-sub","No areas selected. Turn on at least one area in step 02.")); return; }
  activeAreas.forEach((a,idx)=>{
    ...
    items.forEach(c=>{
      ...
      const info=el("div","cinfo",`<div class="ct"><span class="cref">${c.ref}</span>${c.t}</div><div class="cd">${c.d||""}</div>`);
      const rate=el("div","crate");
      rate.appendChild(el("div","expbadge",`Expectation at ${state.level}: <b>${e}</b>`));
      ...
      seg.appendChild(naB);
      const foot=el("div"); foot.style.cssText="display:flex;gap:9px;align-items:center;flex-wrap:wrap;justify-content:flex-end";
      const dp=el("div","deltapill","-"); dp.dataset.ref=c.ref;
      const pv=el("div","prevtag"); pv.dataset.prev=c.ref;
      foot.appendChild(pv); foot.appendChild(dp);
      rate.appendChild(seg); rate.appendChild(foot);
      row.appendChild(info); row.appendChild(rate);
      rows.appendChild(row);
    });
    block.appendChild(rows); m.appendChild(block);
  });
...
function renderLearning(){
  ...
  if(!areas.length){
    wrap.appendChild(el("div","panel-sub", state.onlyGaps
      ...
      : "No areas in scope. Turn areas on in step 02."));
    return;
  }
  areas.forEach(a=>{
    ...
    items.forEach(x=>{
      ...
      const url=PROV_URL[x.p](x.t);
      card.innerHTML=`<span class="prov ${x.p}">${PROV_NAME[x.p]}</span>
